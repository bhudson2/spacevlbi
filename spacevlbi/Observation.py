...
def Baselines(i, spaceTelescopes, groundTelescopes, sourceRa, sourceDec, \
    ...
    # Iterate through observation frequencies
    for f in range(len(frequency)):
        # Calculate wavelength
        wavelength = c.value / frequency[f]
            ...
            for d in range(len(declination)):
                ...
                for j in range(len(spaceTelescopes)):
                    ...
                    if len(spaceTelescopes[j].baselines) == 0:
                        ...
                        matrixLost = []
                        for _ in range(rows):
                            matrix.append(np.zeros((len(frequency), int(round(simLength/timeStep)),\
                                                    int(stationNo)*3)))
                            matrixLost.append(np.zeros((len(frequency), int(round(simLength/timeStep)),\
                                                        int(stationNo)*3)))
                    ...
                    for l in range(len(spaceTelescopes)):
                        ...
                        if j != l:
                            ...
                            if obsFlag == 0 and sourceVis1 == 1 and sourceVis2 == 1:
                                spaceTelescopes[j].baselines[r*len(declination)+d][f]\
                                    [i,baselineCount*3-3:baselineCount*3] = [0,0,0]
                                # Add lost baselines to space telescope property
                                spaceTelescopes[j].lostBaselines[r*len(declination)+d][f]\
                ...
                for j in range(len(groundTelescopes)):
                    ...
                    if len(groundTelescopes[j].baselines) == 0:
                        ...
                        matrixLost = []
                        for _ in range(rows):
                            matrix.append(np.zeros((len(frequency), int(round(simLength/timeStep)),int(stationNo)*3)))
                            matrixLost.append(np.zeros((len(frequency), int(round(simLength/timeStep)),int(stationNo)*3)))
                    ...
                    for l in range(len(groundTelescopes)):
                        ...
                        if j != l:
                            ...
                            # baseline, else calculate baseline
                            if obsFlag == 0 and sourceVis1 == 1 and sourceVis2 == 1:
                                groundTelescopes[j].baselines[r*len(declination)+d][f]\
                                    [i,baselineCount*3-3:baselineCount*3] = [0,0,0]
                                # Add lost baselines to space telescope property
                                groundTelescopes[j].lostBaselines[r*len(declination)+d][f]\
                                    [i,baselineCount*3-3:baselineCount*3] = vector.reshape((1,3))
                            elif obsFlag == 1 and sourceVis1 == 1 and sourceVis2 == 1:
                                # Calculate uvw coordinates
                                groundTelescopes[j].baselines[r*len(declination)+d][f]\
